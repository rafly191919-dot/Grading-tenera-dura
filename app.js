(function () {
  ...
  function init() {
    ...
    if (isLoggedIn()) {
      signInFirebaseAndStartRealtime(true).catch((error) => {
        ...
        setFirebaseStatus("Mode lokal aktif - Firebase belum tersambung", "pending");
      });
    }
  }
  ...
  function showOpenMethodWarning() {
    ...
    if (loginStatusEl) {
      loginStatusEl.innerHTML = "Mode lokal siap. Firebase akan dicoba otomatis saat koneksi/hosting mendukung.";
    }
  ...
  async function signInFirebaseAndStartRealtime(silent = false) {
    ...
    try {
      setFirebaseStatus("Menghubungkan Firebase...", "syncing");
      ...
      setFirebaseStatus("Firebase realtime aktif", "online");
      if (!silent) toast("Firebase realtime aktif.");
    } catch (error) {
      console.error("Firebase sign-in/realtime gagal:", error);
      const detail = firebaseFriendlyError(error);
      setFirebaseStatus("Mode lokal aktif - Firebase belum tersambung", "pending");
      if (!silent) {
        toast("Mode lokal aktif. Firebase belum tersambung, data tetap tersimpan di perangkat ini.", true);
      }
      throw error;
    }
  }
  ...
  async function runFirebaseDiagnostic(showToast = true) {
    try {
      ...
      await testFirestoreConnection();
      firebaseState.ready = true;
      setFirebaseStatus("Firebase database aktif", "online");
  ...
  function queueCloudSync() {
    if (!firebaseState.ready || firebaseState.applyingRemote || !firebaseState.db) return;
  ...
  async function fullCloudSync() {
    ...
    try {
      await syncArrayToFirestore("suppliers", state.suppliers, { allowDelete: firebaseState.snapshotReady.suppliers });
      await syncArrayToFirestore("drivers", state.drivers, { allowDelete: firebaseState.snapshotReady.drivers });
      await syncArrayToFirestore("grading", state.grading, { allowDelete: firebaseState.snapshotReady.grading });
      await syncArrayToFirestore("td", state.td, { allowDelete: firebaseState.snapshotReady.td });
      await syncArrayToFirestore("auditLogs", state.auditLogs, { allowDelete: firebaseState.snapshotReady.auditLogs });
      await firebaseState.db.collection(FIREBASE_SETTINGS_COLLECTION).doc(FIREBASE_SETTINGS_DOC).set(cleanForFirestore(state.settings), { merge: false });
      setFirebaseStatus("Firebase realtime aktif", "online");
    } catch (error) {
  ...
  function setupAuth() {
    ...
    if (loginForm) {
      loginForm.addEventListener("submit", (event) => {
        ...
        setFirebaseStatus("Mode lokal aktif - sinkron Firebase dicoba otomatis", "pending");
        updateAuthVisibility();
        updateRoleUI();
        renderAll();
  ...
  function updateAuthVisibility() {
    ...
    if (!logged && firebaseState.initialized) setFirebaseStatus("Firebase: belum masuk", "pending");
    updateRoleUI();
  }
